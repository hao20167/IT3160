...
from util.replay_buffer_DQN import Multi_step_ReplayBuffer_multi_info
import random
...
import copy
from util.graph import get_test_contrast_curve
from util.episode_selector import (
    start_selector,
...
parser.add_argument(
    "--train_data_path",
    type=str,
    default="data/BTCTUSD/final/train",
...
class DQN(object):
    def __init__(self, args):
        ...
        self.tech_indicator_list = np.load("data/BTCUSDT/feature_analysis/2024-12-17_2025-01-02/second_feature.npy").tolist()
    ...
    def train(self):
        ...
        for i in range(df_number):
            ...
            if i not in self.df_cache:
                self.df_cache[i] = pd.read_feather(
        ...
        for sample in range(self.num_sample):
            ...
            if df_index not in self.df_cache:
                 self.df_cache[df_index] = pd.read_feather(
                    os.path.join(self.train_data_path, "df_{}.feather".format(df_index))
                )
            self.train_df = self.df_cache[df_index]
